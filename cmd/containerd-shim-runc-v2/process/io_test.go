...
func TestNewAttachableBinaryIO(t *testing.T) {
	...
	tempDir := t.TempDir()
	_, err := NewBinaryIO(ctx, "1", filepath.Join(tempDir, "attach-stdout"), filepath.Join(tempDir, "attach-stderr"), uri)
	if err != nil {
		t.Fatal(err)
	}

	after := descriptorCount(t)
	// The value `11` represents the number of file descriptors (FIFOs)
	// opened and kept open when `NewBinaryIO` is called with attachable
	// FIFOs for stdout and stderr.
	if before != after-11 {
