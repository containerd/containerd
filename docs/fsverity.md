...
containerd can use the Linux kernel's `fs-verity` feature to protect on-disk image content. When active, the kernel detects in-place tampering of content-store blobs (layers, configs, etc.) and blocks reads of corrupted data, providing defense-in-depth against [STORE-006](./security/THREAT_MODEL.md#threat-id-store-006) (at-rest blob tampering).
...
While advanced snapshotters (such as EROFS) support optional `fs-verity` configuration knobs (e.g., `enable_fsverity`), no configuration is required inside containerd for the default local content store. Instead, containerd opportunistically enables the feature when the host kernel and filesystem support it. If enabling `fs-verity` on a blob fails, containerd logs a warning and continues — the write is **not** integrity-protected in that case, so do not assume every blob is covered.
...
What `fs-verity` does **not** do, and common misconceptions:

*   **Replacement is still possible.** A `root` attacker can delete the protected file and write a new one with its own `fs-verity` measurement. This is **not** caught by `fs-verity`; it is caught by containerd's **content-addressable** store, which detects that the new blob's digest no longer matches — a property that holds with or without `fs-verity`.
...
Once the kernel and filesystem are correctly configured, `containerd` will automatically attempt to enable `fs-verity` on new content written to the default content store. No further configuration is required for standard local storage, though optional snapshotters (like EROFS) must be configured explicitly. As noted above, `containerd` logs a warning and continues if it fails to enable `fs-verity` on a supported filesystem, so coverage is best-effort. There is currently no explicit command to check whether a specific blob has `fs-verity` enabled through the `ctr` tool.
