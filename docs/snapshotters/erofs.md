...
used together to apply image layers.  Otherwise, the EROFS snapshotter should
just behave as the existing OverlayFS snapshotter: the default applier will
unpack the current image layer into the active EROFS snapshot, and commit it.
Also see _the [Configuration](#configuration) section_.
