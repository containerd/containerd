...
containerd's role is to execute the container specification exactly as requested by the CRI client (e.g., Kubelet). If the Kubelet configures a workload with broad privileges (e.g., host namespaces, root execution, or privileged volume mounts), containerd will fulfill it by design.
...
*   **Class 2 (attacker-influenced content forwarded verbatim):** The image reference, the image bytes (layers), the values embedded in the image config (labels, environment, declared volumes, entrypoint, the `USER` directive), and the contents of container checkpoint archives imported through CRI. The Kubelet does not author or inspect this content. It forwards a name or an archive, and containerd pulls and interprets the bytes.
