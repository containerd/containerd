...
The `cri` plugin uses containerd to manage the full container lifecycle and all container images; it consumes containerd through an in-process client rather than over the gRPC socket. As also shown below, `cri` manages pod networking via [CNI](https://github.com/containernetworking/cni) (another CNCF project).
