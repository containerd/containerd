...
It runs golangci-lint and verifies protobuf formatting.
...
`make check-protos` verifies the protobuf files are properly formatted (`make check` includes it).
...
- `internal` - All utility packages used by containerd and not intended for direct import; the bulk of the CRI implementation lives in `internal/cri`
...
- `plugins` - All included containerd plugins which are registered via init; `plugins/types.go` holds the plugin type constants
