...
func (c *criService) stopContainer(ctx context.Context, container containerstore.Container, timeout time.Duration) error {
	...
	if timeout > 0 {
		...
		if container.StopSignal != "" {
			...
			if err != nil {
				return fmt.Errorf("failed to get runtime handler string from container's sandbox with ID %v", container.SandboxID)
			}
			image, err := c.imageStore.Get(container.ImageRef, sb.RuntimeHandler)
			if err != nil {
				if !errdefs.IsNotFound(err) {
