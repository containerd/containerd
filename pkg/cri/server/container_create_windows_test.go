...
func TestEntrypointAndCmdForArgsEscaped(t *testing.T) {
	...
	for name, test := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			...
			if err := assertEqualsStringArrays(actualArgs, test.expectedArgs); err != nil {
				t.Fatal(err)
			}
			if actualCommandLine != test.expectedCommandLine {
				t.Fatalf("Expected CommandLine: %s, got: '%s'", test.expectedCommandLine, actualCommandLine)
			}
		})
	}
}
