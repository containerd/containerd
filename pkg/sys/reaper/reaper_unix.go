...
func (sl *SafeList) Push(value interface{}) {
	sl.mu.Lock()
	defer sl.mu.Unlock()
...
func (sl *SafeList) Pop() interface{} {
	sl.mu.Lock()
	defer sl.mu.Unlock()
	...
	for {
		...
		if exitEvt, ok := event.(eventExit); ok {
			...
			case <-time.After(10 * time.Second):
				log.L.Warnf("event with pid %d status:%d may may be lost", exitEvt.Pid, exitEvt.Status)
			}
		}
	}
}
