...
func (e *Emitter) Events(ctx context.Context, clientID string, fs ...filters.Filter) chan *events.Envelope {
	eventFilters := filters.Filter(filters.Any(fs))
	if len(fs) == 0 {
		eventFilters = filters.Always
	}
