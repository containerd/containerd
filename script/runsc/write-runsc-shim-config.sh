...
# containerd-shim-runsc-v1 expects systemd-cgroup as a TOML string ("true" /
# "false"), not a bare boolean — its config struct maps the field to a Go string.
case "${1:-false}" in
...
# The runsc shim checks these paths in order (higher precedence first):
#   /run/containerd/runsc/config.toml
#   /etc/containerd/runsc/config.toml
# Remove the higher-precedence runtime path first so that a stale file from a
# previous run cannot silently override the settings we are about to write.
