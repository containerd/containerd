...
func (s *snapshotter) Stat(ctx context.Context, key string) (snapshots.Info, error) {
	...
	if err := view(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if err != nil {
			return fmt.Errorf("failed to read labels: %w", errdefs.ErrNotFound)
		}
		if err := boltutil.ReadTimestamps(sbkt, &local.Created, &local.Updated); err != nil {
			return fmt.Errorf("failed to read timestamps: %w", errdefs.ErrNotFound)
		}
...
func (s *snapshotter) Update(ctx context.Context, info snapshots.Info, fieldpaths ...string) (snapshots.Info, error) {
	...
	if info.Name == "" {
		return snapshots.Info{}, fmt.Errorf("%w", errdefs.ErrInvalidArgument)
	}
...
func (s *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) error {
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if bkt == nil {
			return fmt.Errorf(
				"can not find snapshotter %q: %w", s.name, errdefs.ErrNotFound)
		}
...
func (s *snapshotter) Remove(ctx context.Context, key string) error {
	...
	return update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		if len(parent) > 0 {
			pbkt := bkt.Bucket(parent)
			if pbkt == nil {
				return fmt.Errorf("parent snapshot %v does not exist: %w", string(parent), errdefs.ErrInvalidArgument)
			}
