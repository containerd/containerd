...
func TestImagePullWith429ExhaustedRetries(t *testing.T) {
	...
	assert.WithinDuration(t, time.Now(), *statusErr.RetryAfter, 5*time.Second)
...
func TestImagePullWith429ExcessiveRetryAfter(t *testing.T) {
	...
	require.True(t, errors.As(err, &statusErr))
	require.NotNil(t, statusErr.RetryAfter)
	assert.WithinDuration(t, time.Now().Add(300*time.Second), *statusErr.RetryAfter, 5*time.Second)
	assert.Contains(t, err.Error(), "(Retry-After: "+statusErr.RetryAfter.Format(time.RFC3339)+")")
}
