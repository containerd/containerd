...
func TestContainerTTYLeakAfterExit(t *testing.T) {
	...
	if criCfg.ContainerdConfig.Runtimes[criCfg.ContainerdConfig.DefaultRuntimeName].Type == "io.containerd.runtime.v1.linux" {
		t.Skip("TTY exit leak regression was shimv2 only")
	}
...
func getShimPid(t *testing.T, sb string) int {
	...
	shimCli := newShimCli(ctx, t, sb, false)
	return int(shimCli.pid(ctx, t))
}
