...
func TestImageTagWithoutRegistryNotVisibleInCRI(t *testing.T) {
	...
	require.NoError(t, err)
	defer func() {
		if err := containerdClient.ImageService().Delete(ctx, nameNoRegistry, containerd.WithDeleteMissing); err != nil {
			assert.True(t, errdefs.IsNotFound(err), err)
		}
		if err := containerdClient.ImageService().Delete(ctx, fullNameNoRegistry, containerd.WithDeleteMissing); err != nil {
			assert.True(t, errdefs.IsNotFound(err), err)
		}
		// TODO: I think this shouldn't even be in the image service because it's lacking a registry.
		if _, err := imageService.RemoveImage(context.Background(), &runtime.RemoveImageRequest{Image: &runtime.ImageSpec{Image: nameNoRegistry}}); err != nil {
			assert.True(t, errdefs.IsNotFound(err), err)
		}
		if _, err = imageService.RemoveImage(context.Background(), &runtime.RemoveImageRequest{Image: &runtime.ImageSpec{Image: fullNameNoRegistry}}); err != nil {
			assert.True(t, errdefs.IsNotFound(err), err)
		}
	}()
