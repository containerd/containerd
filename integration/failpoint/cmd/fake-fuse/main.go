...
var fuseDir = flag.String("fuse-dir", "", "dir to be used by the FUSE process to store everything.")
...
func main() {
	...
	// Create the user-provided FUSE dir if it does not exist.
	if err := os.MkdirAll(*fuseDir, os.ModeDir); err != nil {
		log.G(ctx).WithField("fuse_dir", *fuseDir).WithError(err).Error("failed to create a FUSE directory")
	}
	...
	if err := fuseStruct.fuseCloseWait.WaitCtx(ctx); err != nil {
...
func createFUSEFS(ctx context.Context) fakeFUSE {
	...
	if *hangDuringStat {
		...
		attrFn = func(ctx context.Context, a *fuse.Attr) error {
			...
			if err := statHangWait.WaitCtx(ctx); err != nil {
...
// Close closes the FUSE connection. The FUSE dir (provided by the user) is not deleted.
...
func (fs fakeFUSE) Root() (fusefs.Node, error) {
	...
	if err := fs.fuseReadyTrigger.Trigger(); err != nil {
