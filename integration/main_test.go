...
// runtimeHandlerIsRunsc reports whether the active runtime handler targets
// the runsc (gVisor) shim. It checks the explicit --runtime-type flag first
// (io.containerd.runsc.v1), then falls back to the handler name. Relying on
// the handler name alone is unreliable because operators commonly register
// runsc under alternative names such as "gvisor" (see docs/cri/config.md),
// so both the documented aliases and the "runsc"/"runsc-*" prefix are matched.
