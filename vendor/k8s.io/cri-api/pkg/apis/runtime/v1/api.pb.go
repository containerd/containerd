...
const (
	...
	// CGROUP_MOUNT_MODE_WRITABLE mounts the cgroup filesystem read-write to let
	// the container create its own cgroups. The runtime requires that cgroup v2
	// is mounted with nsdelegate, which prevents a container from changing its
	// own cgroup limits; otherwise, the runtime MUST fail container creation.
