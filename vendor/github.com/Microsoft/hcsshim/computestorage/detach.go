...
// DetachUnionFSFilter detaches the unionFS filter on a writable container layer.
//
// `volumePath` is a path to writable container volume.
func DetachUnionFSFilter(ctx context.Context, volumePath string) (err error) {
	title := "hcsshim::DetachUnionFSFilter"
	ctx, span := oc.StartSpan(ctx, title) //nolint:ineffassign,staticcheck
	defer span.End()
	defer func() { oc.SetSpanStatus(span, err) }()
	...
	layerData := LayerData{}
	layerData.FilterType = hcsschema.UnionFS
	...
	if err != nil {
		return errors.Wrap(err, "failed to detach unionFS filter")
	}
	return nil
}
