...
func evalSymlinksUsingGetFinalPathNameByHandle(path string) (string, error) {
	err := syscallx.LoadGetFinalPathNameByHandle()
	if err != nil {
	...
	buf := make([]uint16, 100)
	for {
		n, err := syscallx.GetFinalPathNameByHandle(h, &buf[0], uint32(len(buf)), syscallx.VOLUME_NAME_GUID)
		if err != nil {
			return "", err
		}
		if n < uint32(len(buf)) {
	...
	fmt.Printf("GetFinalPathNameByHandle: %s\n", s)
	if len(s) > 4 && s[:4] == `\\?\` {
		if len(s) > 48 && s[:11] == `\\?\Volume{` {
			// return path like \\?\Volume{GUID}
			return s, nil
		}
		s = s[4:]
		if len(s) > 3 && s[:3] == `UNC` {
...
func evalSymlinks(path string) (string, error) {
	...
	if err != nil {
		fmt.Printf("walkSymlinks err: %v\n", err)
		newpath2, err2 := evalSymlinksUsingGetFinalPathNameByHandle(path)
		if err2 == nil {
			fmt.Printf("evalSymlinksUsingGetFinalPathNameByHandle returned: %s\n", newpath2)
