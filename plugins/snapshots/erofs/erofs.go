...
type SnapshotterConfig struct {
	...
	defaultSize int64
	// mergeFsmeta enables EROFS fsmerge feature
	mergeFsmeta bool
}
...
// WithMergeFSMeta enables EROFS fsmerge feature
func WithMergeFSMeta() Opt {
	return func(config *SnapshotterConfig) {
		config.mergeFsmeta = true
	}
}
...
func (s *snapshotter) mountFsMeta(snap storage.Snapshot, id int, mounts *[]mount.Mount) bool {
	...
	for j := len(snap.ParentIDs) - 1; j >= id; j-- {
		...
		*mounts = append(*mounts, mount.Mount{
		...
		m.Options = append(m.Options, fmt.Sprintf("device={{ mount %d }}", len(*mounts)-1))
	}
...
func (s *snapshotter) mounts(snap storage.Snapshot, _ snapshots.Info) ([]mount.Mount, error) {
	...
	for i := range snap.ParentIDs {
		...
		if ok := s.mountFsMeta(snap, i, &mounts); ok {
			first = len(mounts) - 1
			break
		}
...
func (s *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) (_ []mount.Mount, err error) {
	...
	if err := s.ms.WithTransaction(ctx, true, func(ctx context.Context) (err error) {
		...
		if !strings.Contains(key, snapshots.UnpackKeyPrefix) && len(snap.ParentIDs) > 0 {
...
func (s *snapshotter) generateFsMeta(ctx context.Context, snapIDs []string) {
	...
	mergedMeta := filepath.Join(s.root, "snapshots", snapIDs[0], "fsmeta.erofs")
	if _, err := os.Stat(mergedMeta); err == nil {
		return
	}
