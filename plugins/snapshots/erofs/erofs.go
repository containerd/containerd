...
func (s *snapshotter) mountFsMeta(snap storage.Snapshot, id int) (bool, mount.Mount) {
	...
	for j := len(snap.ParentIDs) - 1; j >= id; j-- {
		...
		m.Options = append(m.Options, fmt.Sprintf("device=%s", path))
	}
	return true, m
}
...
func (s *snapshotter) mounts(snap storage.Snapshot, _ snapshots.Info) ([]mount.Mount, error) {
	...
	for i := range snap.ParentIDs {
		...
		if s.fsMergeThreshold > 0 {
			if ok, m := s.mountFsMeta(snap, i); ok {
...
func (s *snapshotter) generateFsMeta(ctx context.Context, snapIDs []string) {
	...
	args := append([]string{"--aufs", "--ovlfs-strip=1", "--quiet", mergedMeta + ".tmp"}, blobs...)
	log.G(ctx).Infof("merging layers with mkfs.erofs %v", args)
	cmd := exec.CommandContext(ctx, "mkfs.erofs", args...)
	_, err := cmd.CombinedOutput()
	if err != nil {
		log.G(ctx).Infof("failed to generate merged fsmeta for %v", snapIDs[0])
		return
	}
	// Atomically replace the fsmeta with the generated file
	if err = os.Rename(mergedMeta+".tmp", mergedMeta); err != nil {
