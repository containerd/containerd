...
// It reads dm-verity metadata from the .dmverity file and includes all necessary parameters in mount options.
func (s *snapshotter) createDmverityErofsMount(id string, layerBlob string) (mount.Mount, error) {
	...
	metadata, err := dmverity.ReadMetadata(layerBlob)
	if err != nil {
		return mount.Mount{}, fmt.Errorf("failed to read dm-verity metadata for layer %s: %w", layerBlob, err)
	}
