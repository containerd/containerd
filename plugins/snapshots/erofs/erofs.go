...
func WithMergeFSMeta(enable bool) Opt {
	return func(config *SnapshotterConfig) {
		config.mergeFsmeta = enable
	}
}
...
func (s *snapshotter) lowerPath(id string) (mount.Mount, string) {
	// First, let's check if a specific "layer.erofs" generated by the EROFS
	// differ exists.  If exists, use EROFS filesystem for this layer.
	...
	// Otherwise, return a pseudo bind mount as an OverlayFS lowerdir since
	// the current active differ may NOT be the EROFS differ.
	return mount.Mount{
		Source:  s.upperPath(id),
		Type:    "bind",
...
func (s *snapshotter) MountFsMeta(snap storage.Snapshot, mnt string, id int) bool {
...
func (s *snapshotter) mounts(snap storage.Snapshot, info snapshots.Info, skipmount bool) ([]mount.Mount, error) {
	var options []string
	...
	for i := range len(snap.ParentIDs) {
		m, mntpoint := s.lowerPath(snap.ParentIDs[i])

		// If the lowerdir is actually an EROFS committed layer (or merged
		// meta) but doesn't have an EROFS mount. Let's recover now.
		if !skipmount && mntpoint != m.Source && !isErofs(mntpoint) {
			// If a merged fsmeta is available in this layer, skip the bottom layers.
			// Why? Because all bottom layers have been flattened with the thin fsmeta.
			if s.MountFsMeta(snap, mntpoint, i) {
		...
		lowerdirs = append(lowerdirs, mntpoint)
		// An EROFS merged meta is mounted, skip the bottom layers too.
		if _, err := os.Stat(s.fsMetaPath(snap.ParentIDs[i])); err == nil {
			break
		}
	}
...
func (s *snapshotter) GenerateFsMeta(ctx context.Context, snap storage.Snapshot) {
	var blobs []string
	...
	mergedMeta := filepath.Join(s.root, "snapshots", snap.ParentIDs[0], "fsmeta.erofs")
	if _, err := os.Stat(mergedMeta); err == nil {
		return
	}
	...
	for i := len(snap.ParentIDs) - 1; i >= 0; i-- {
		...
		// It's impossible if any layer blob is missing, mount one-by-one as fallback
	...
	if err != nil {
		log.G(ctx).Infof("failed to generate merged fsmeta for %v", snap.ID)
	}
}
...
func (s *snapshotter) createSnapshot(ctx context.Context, kind snapshots.Kind, key, parent string, opts []snapshots.Opt) (_ []mount.Mount, err error) {
	...
	return s.mounts(snap, info, imagePull)
}
...
func (s *snapshotter) Commit(ctx context.Context, name, key string, opts ...snapshots.Opt) error {
	return s.ms.WithTransaction(ctx, true, func(ctx context.Context) error {
		...
		usage, err := fs.DiskUsage(ctx, s.upperPath(id))
		if err != nil {
			return err
		}
...
func (s *snapshotter) Mounts(ctx context.Context, key string) (_ []mount.Mount, err error) {
	...
	return s.mounts(snap, info, false)
}
