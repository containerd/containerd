...
const deviceCleanupDelay = 10 * time.Second

type erofsMountHandler struct {
	mu              sync.Mutex
	dmverityDevices map[string]string      // mountPoint -> deviceName
	pendingCleanups map[string]*time.Timer // deviceName -> cleanup timer
}
...
func NewErofsMountHandler() mount.Handler {
	return &erofsMountHandler{
		dmverityDevices: make(map[string]string),
...
func (h *erofsMountHandler) Mount(ctx context.Context, m mount.Mount, mp string, _ []mount.ActiveMount) (mount.ActiveMount, error) {
	...
	if err == nil && dmverityMode != "off" {
		// Layer has dm-verity metadata and mode allows it - set up dm-verity device
		...
		if _, err := os.Stat(devicePath); os.IsNotExist(err) {
			...
			devicePath, err = dmverity.Open(m.Source, deviceName, m.Source, metadata.RootHash, metadata.HashOffset, nil)
			if err != nil {
			...
			h.mu.Lock()
			if timer, exists := h.pendingCleanups[deviceName]; exists {
				timer.Stop()
				delete(h.pendingCleanups, deviceName)
				log.G(ctx).WithField("device", deviceName).Debug("cancelled pending cleanup for reused device")
			}
			h.mu.Unlock()
		}

		// Update mount source to point to dm-verity device
		m.Source = devicePath
	}
	...
	// Store dm-verity device name for cleanup on unmount
	if dmverityDevice != "" {
		h.mu.Lock()
		h.dmverityDevices[mp] = dmverityDevice
		h.mu.Unlock()
		log.G(ctx).WithFields(log.Fields{
			"mount-point": mp,
			"device":      dmverityDevice,
			"source":      originalSource,
		}).Debug("stored dm-verity device for cleanup")
	}
...
func (h *erofsMountHandler) Unmount(ctx context.Context, path string) error {
	...
	// Clean up dm-verity device if this mount used one
	h.mu.Lock()
	deviceName, hasDmverity := h.dmverityDevices[path]
	if hasDmverity {
		delete(h.dmverityDevices, path)
	}
	h.mu.Unlock()

	if hasDmverity {
		// Schedule delayed cleanup - kernel will refuse to remove if device is still mounted
		log.G(ctx).WithFields(log.Fields{
			"mount-point": path,
			"device":      deviceName,
			"delay":       deviceCleanupDelay,
		}).Debug("scheduling delayed cleanup for dm-verity device")

		h.mu.Lock()
		// Cancel any existing cleanup timer for this device
		if existingTimer, exists := h.pendingCleanups[deviceName]; exists {
			existingTimer.Stop()
		}

		// Schedule new cleanup
		timer := time.AfterFunc(deviceCleanupDelay, func() {
			log.G(ctx).WithField("device", deviceName).Debug("executing delayed cleanup for dm-verity device")

			if closeErr := dmverity.Close(deviceName); closeErr != nil {
				log.G(ctx).WithError(closeErr).WithField("device", deviceName).Warn("failed to close dm-verity device during delayed cleanup")
			} else {
				log.G(ctx).WithField("device", deviceName).Debug("dm-verity device closed successfully during delayed cleanup")
			}
		...
		h.pendingCleanups[deviceName] = timer
		h.mu.Unlock()
	}
