...
func init() {
	...
	registry.Register(&plugin.Registration{
		...
		InitFn: func(ic *plugin.InitContext) (interface{}, error) {
			...
			// TODO(ambarve): update this to check if blocked CIMs are supported
...
// TODO(ambarve): make these type part of a single public repo and consume from there
const (
	BlockCIMMountType       string = "BlockCIM"
	BlockCIMTypeFlag        string = "blockCIMType="
	parentLayerCimPathsFlag string = "parentCimPaths="
)
...
func parseBlockCIMMount(m *mount.Mount) (*cimfs.BlockCIM, []*cimfs.BlockCIM, error) {
	...
	for _, option := range m.Options {
		if val, ok := strings.CutPrefix(option, parentLayerCimPathsFlag); ok {
			...
			if val != "file" {
				return nil, nil, fmt.Errorf("extraction doesn't support layer type `%s`", layerType)
			}
		}
	}
...
func (c blockCIMDiff) Apply(ctx context.Context, desc ocispec.Descriptor, mounts []mount.Mount, opts ...diff.ApplyOpt) (d ocispec.Descriptor, err error) {
	if len(mounts) != 1 {
		return emptyDesc, fmt.Errorf("number of mounts should always be 1 for CimFS layers: %w", errdefs.ErrInvalidArgument)
	} else if mounts[0].Type != "BlockCIM" {
