...
func ValidateRuntimeConfig(ctx context.Context, c *RuntimeConfig) ([]deprecation.Warning, error) {
	...
	if c.StatsCollectPeriod != "" {
		...
		if d <= 0 {
			return warnings, fmt.Errorf("invalid `stats_collect_period`: duration must be greater than zero")
		}
	...
	if c.StatsRetentionPeriod != "" {
		...
		if d <= 0 {
			return warnings, fmt.Errorf("invalid `stats_retention_period`: duration must be greater than zero")
		}
