...
func (c *CheckpointService) recoverMissingCheckpointCgroup(ctx context.Context, marker podCheckpointRecoveryMarker) error {
	if _, err := c.client.LoadContainer(ctx, marker.SandboxID); err == nil {
		return fmt.Errorf("pod checkpoint recovery cgroup is missing but sandbox container %q still exists", marker.SandboxID)
	} else if !errdefs.IsNotFound(err) {
		return fmt.Errorf("failed to verify deleted checkpoint sandbox container %q: %w", marker.SandboxID, err)
	}
	for _, containerID := range marker.ContainerIDs {
	...
	log.G(ctx).WithField("sandbox", marker.SandboxID).Info("Removed checkpoint recovery marker for deleted pod")
	return nil
}
