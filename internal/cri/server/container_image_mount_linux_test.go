...
func TestClearImageVolumeMountMappings(t *testing.T) {
	mappings := []*runtime.IDMapping{
		{
			ContainerId: 0,
			HostId:      65536,
	...
	// Verify mappings exist before clearing
	assert.NotNil(t, mount.UidMappings)
	assert.NotNil(t, mount.GidMappings)

	clearImageVolumeMountMappings(mount)

	// Verify mappings are cleared
	assert.Nil(t, mount.UidMappings, "UidMappings should be cleared")
	assert.Nil(t, mount.GidMappings, "GidMappings should be cleared")
}
