...
func (c *CRIImageService) PullImage(ctx context.Context, name string, credentials func(string) (string, string, error), sandboxConfig *runtime.PodSandboxConfig, runtimeHandler string) (_ string, err error) {
	...
	if c.config.UseLocalImagePull {
		image, isSchema1, err = c.pullImageWithLocalPull(ctx, ref, credentials, snapshotter, labels, imagePullProgressTimeout)
	} else {
		if c.config.MaxConcurrentDownloads > 0 {
			log.G(ctx).Warn("'max_concurrent_downloads' should be configured under [plugins.\"io.containerd.transfer.v1.local\"] when using transfer service")
		}
...
func (c *CRIImageService) pullImageWithLocalPull(
	...
	pcancel()
	if err != nil {
		return nil, false, fmt.Errorf("failed to pull and unpack image %q: %w", ref, err)
	}
	return image, isSchema1, nil
}
