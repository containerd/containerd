...
func (c *criService) portForward(ctx context.Context, id string, port int32, stream io.ReadWriteCloser) error {
	...
	if err := validateSkipLocalhostForPortForward(skipLocalhost, podIPs); err != nil {
		return fmt.Errorf("port forwarding sandbox %q with runtime %q: %w", id, ociRuntime.Type, err)
	}
	...
	if err != nil && len(podIPs) > 0 {
		if skipLocalhost {
			...
			log.G(ctx).Debugf("localhost dial failed for sandbox %q port %d (%v), falling back to pod IPs %v in network namespace %q", id, port, err, podIPs, netNSPath)
			err = netNSDo(func(_ ns.NetNS) error {
				var dialErr error
				conn, dialErr = dialPodIPs(ctx, podIPs, port)
				return dialErr
			})
		}
	}
	if err != nil {
		return fmt.Errorf("failed to establish portforward connection for sandbox %q: %w", id, err)
	}
...
func validateSkipLocalhostForPortForward(skipLocalhost bool, podIPs []string) error {
	if skipLocalhost && len(podIPs) == 0 {
		return fmt.Errorf("configured to skip localhost requires a pod IP")
	}
	return nil
}
