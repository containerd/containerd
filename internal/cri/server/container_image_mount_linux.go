...
// clearImageVolumeMountMappings clears the UID/GID mappings from the mount.
// This must be called for image volume mounts with user namespace mappings to prevent
// the OCI runtime from attempting idmap on the bind mount, since containerd already
// applies idmap to the overlay lower layers via the snapshotter.
func clearImageVolumeMountMappings(mount *runtime.Mount) {
	mount.UidMappings = nil
	mount.GidMappings = nil
}
