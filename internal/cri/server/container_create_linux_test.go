...
func TestMountPropagation(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			err := opts.WithMounts(c.os, c.osManager, config, []*runtime.Mount{test.criMount}, "", nil)(context.Background(), nil, nil, &spec)
			if test.expectErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestIssue10828(t *testing.T) {
	...
	defer func() {
		fuseCmd.Cancel()
		// fuseCmdErr := fuseCmd.Wait()
		// if fuseCmdErr != nil {
		// 	t.Logf("FUSE process exited with error: %v\n", fuseCmdErr)
		// 	t.Logf("FUSE process stdout:\n%s\n", outBuffer.String())
		// 	t.Logf("FUSE process stderr:\n%s\n", errBuffer.String())
		// }
	}()
