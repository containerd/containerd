...
import (
	...
	osi "github.com/containerd/containerd/v2/pkg/os"
...
func TestContainerSpecWithExtraMounts(t *testing.T) {
	...
	spec, err := c.buildContainerSpec(t.Context(), currentPlatform, testID, testSandboxID, testPid, "", testContainerName, testImageName, containerConfig, sandboxConfig, imageConfig, extraMounts, ociRuntime, nil)
	require.NoError(t, err)
	specCheck(t, testID, testSandboxID, testPid, spec)
...
func TestPrivilegedBindMount(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			spec, err := c.buildContainerSpec(t.Context(), currentPlatform, t.Name(), testSandboxID, testPid, "", testContainerName, testImageName, containerConfig, sandboxConfig, imageConfig, nil, ociRuntime, nil)
...
func TestIssue10828(t *testing.T) {
	...
	fuseCmd := exec.CommandContext(context.Background(), fuseRunCmd, "--fuse-dir", testDir, "--hang-during-stat")
	err := fuseCmd.Start()
	require.NoErrorf(t, err, "failed to start the FUSE process via %q", fuseRunCmd)

	t.Cleanup(func() {
		// Resume the hanging Stat call before finishing the test
		statHangResume, err := fifosync.NewTrigger(statHangResumeFIFOFile, 0600)
		require.NoErrorf(t, err, "failed to create a trigger to resume the Stat call: %v", statHangResume.Name())
	...
	// TEST
	c := newTestCRIService()
	c.os.(*ostesting.FakeOS).StatFn = osi.RealOS{}.Stat
	...
	go func() {
		_, err := c.buildContainerSpec(t.Context(), currentPlatform, "testID-1", "testSandboxID-1", uint32(1234), "", "ctr-1", testImageName, container1Config, sandbox1Config, imageConfig, nil, config.Runtime{}, nil)
		err1Ch <- err
	}()

	var err2Ch = make(chan error, 1)
	go func() {
		_, err := c.buildContainerSpec(t.Context(), currentPlatform, "testID-2", "testSandboxID-1", uint32(1234), "", "ctr-2", testImageName, container1Config, sandbox1Config, imageConfig, nil, config.Runtime{}, nil)
		err2Ch <- err
	}()
	...
	go func() {
		_, err := c.buildContainerSpec(t.Context(), currentPlatform, "testID-3", "testSandboxID-3", uint32(2345), "", "ctr-3", testImageName, container3Config, sandbox3Config, imageConfig, nil, config.Runtime{}, nil)
		err3Ch <- err
	}()
	...
	select {
	case err1 := <-err1Ch:
		require.Error(t, err1, "ctr-1 should have failed to create")
	case <-time.After(statTimeout + 50*time.Millisecond):
		t.Errorf("stat (err1) did not return any result within the given timeout")
	}

	select {
	case err2 := <-err2Ch:
		require.Error(t, err2, "ctr-2 should have failed to create")
	case <-time.After(statTimeout + 50*time.Millisecond):
		t.Errorf("stat (err2) did not return any result within the given timeout")
	}

	select {
	case err3 := <-err3Ch:
		require.NoError(t, err3, "ctr-3 should not have failed to create")
	case <-time.After(statTimeout + 50*time.Millisecond):
		t.Errorf("stat (err3) did not return any result within the given timeout")
	}
}
...
func TestNonRootUserAndDevices(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.desc, func(t *testing.T) {
			...
			spec, err := c.buildContainerSpec(t.Context(), currentPlatform, t.Name(), testSandboxID, testPid, "", testContainerName, testImageName, containerConfig, sandboxConfig, imageConfig, nil, config.Runtime{}, nil)
			assert.NoError(t, err)
