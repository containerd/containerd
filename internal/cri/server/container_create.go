	...
	if runtimeHandler != "" {
		...
		if err == nil && ociRuntime.Snapshotter != "" {
			...
			if err != nil {
				...
				log.G(ctx).Debugf("Image %q exists but not unpacked for snapshotter %q, unpacking with ref %q", imageRef, ociRuntime.Snapshotter, pullableRef)
				if err := c.ImageService.UnpackImage(ctx, pullableRef, ociRuntime.Snapshotter); err != nil {
					return nil, fmt.Errorf("failed to unpack image %q for snapshotter %q: %w", pullableRef, ociRuntime.Snapshotter, err)
				}
			}
		}
	}
