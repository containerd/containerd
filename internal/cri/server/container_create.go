...
func (c *criService) CreateContainer(ctx context.Context, r *runtime.CreateContainerRequest) (_ *runtime.CreateContainerResponse, retErr error) {
	...
	if checkpointImage {
		...
		log.G(ctx).Infof("Prepared %s for restore\n", ctrID)
...
func (c *criService) createContainer(r *createContainerRequest) (_ string, retErr error) {
	...
	ociRuntime, err := c.config.GetSandboxRuntime(r.podSandboxConfig, r.sandboxRuntimeHandler)
	if err != nil {
	...
	containerLabels := buildLabels(r.containerConfig.Labels, r.imageConfig.Labels, crilabels.ContainerKindContainer)
