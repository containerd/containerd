...
func parseMount(ctx context.Context, osi osinterface.OS, statManager *osinterface.StatManager, mount *runtime.Mount) (*runtimespec.Mount, error) {
	var (
...
// windows container.
func WithWindowsMounts(osi osinterface.OS, statManager *osinterface.StatManager, config *runtime.ContainerConfig, extra []*runtime.Mount) oci.SpecOpts {
	return func(ctx context.Context, client oci.Client, _ *containers.Container, s *runtimespec.Spec) error {
		...
		for _, mount := range mounts {
			parsedMount, err := parseMount(ctx, osi, statManager, mount)
			if err != nil {
				return err
			}
