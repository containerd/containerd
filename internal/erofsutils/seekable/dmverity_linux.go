...
// MaterializeDMVerity extracts the dm-verity payload from a seekable EROFS blob,
// appends it to layer.erofs, and writes a dmverity.DmverityMetadata sidecar file.
//
// rootDigest is the OCI annotation value in "algo:hex" format (e.g. "sha512:abcdef...").
// The algo prefix is stripped before writing the sidecar, since the upstream mount handler
// (dmverity.Open -> utils.ParseRootHash) expects plain hex.
//
...
func MaterializeDMVerity(
	...
	// Strip the "algo:" prefix from the OCI annotation digest format.
	// The upstream mount handler passes RootHash directly to utils.ParseRootHash,
	// which expects plain hex (not "sha512:hex").
	rootHash := rootDigest
	if _, hex, ok := strings.Cut(rootDigest, ":"); ok {
		rootHash = hex
	}

	meta := dmverity.DmverityMetadata{
		RootHash:   rootHash,
		HashOffset: uint64(dmVerityOffsetInLayer),
	}
	if err := dmverity.WriteMetadata(layerPath, &meta); err != nil {
		return err
	}
	return nil
}
