...
func Open(dataDevice string, name string, hashDevice string, rootHash string, hashOffset uint64, opts *DmverityOptions) (string, error) {
	...
	// Setup loop devices for data and hash
	...
	if hashDevice != dataDevice {
		...
		if err != nil {
			// Clean up data loop device on error
			dataLoop.Close()
	...
	// Use library's VerityOpen to handle dm-verity device creation
	devicePath, err := verity.VerityOpen(&params, name, dataLoopDevice, hashLoopDevice, rootDigest, "", nil)
	if err != nil {
		// Clean up loop devices on error
		dataLoop.Close()
		mount.DetachLoopDevice(dataLoopDevice)
		if hashLoop != nil {
			hashLoop.Close()
