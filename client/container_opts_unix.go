...
func withRemappedSnapshotBase(id string, i Image, uidmaps, gidmaps []specs.LinuxIDMapping, readonly bool) NewContainerOpts {
	return func(ctx context.Context, client *Client, c *containers.Container) error {
		...
		rsn := remappedSnapshot{
			...
			IDMaps: sys.IdentityMapping{UIDMaps: uidmaps, GIDMaps: gidmaps},
		}
		usernsID, err := rsn.ID()
		if err != nil {
...
func remapRootFS(ctx context.Context, mounts []mount.Mount, idMap sys.IdentityMapping) error {
	return mount.WithTempMount(ctx, mounts, func(root string) error {
		return filepath.Walk(root, chown(root, idMap))
	})
}

func chown(root string, idMap sys.IdentityMapping) filepath.WalkFunc {
	return func(path string, info os.FileInfo, err error) error {
		if err != nil {
			return err
		}
		stat := info.Sys().(*syscall.Stat_t)
		h, cerr := idMap.ToHost(sys.Identity{UID: stat.Uid, GID: stat.Gid})
		if cerr != nil {
			return cerr
		}
