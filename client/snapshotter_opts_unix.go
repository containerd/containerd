...
func resolveSnapshotOptions(ctx context.Context, client *Client, snapshotterName string, snapshotter snapshots.Snapshotter, parent string, opts ...snapshots.Opt) (string, error) {
	...
	rsn := remappedSnapshot{
		Parent: parent,
		IDMaps: userns.IdentityMapping{
			UIDMaps: []specs.LinuxIDMapping{uidMap},
			GIDMaps: []specs.LinuxIDMapping{gidMap},
		},
	}
	usernsID, err := rsn.ID()
	if err != nil {
...
type remappedSnapshot struct {
	...
	IDMaps userns.IdentityMapping `json:"IDMaps"`
}
...
func (s *remappedSnapshot) ID() (string, error) {
	...
	slices.SortStableFunc(s.IDMaps.UIDMaps, compare)
	slices.SortStableFunc(s.IDMaps.GIDMaps, compare)
