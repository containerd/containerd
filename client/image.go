...
func (i *image) checkSnapshotterSupport(ctx context.Context, snapshotterName string, manifest ocispec.Manifest, forceCheck bool) error {
	manifestPlatform, err := images.ConfigPlatform(ctx, i.ContentStore(), manifest.Config)
	if err != nil {
		return err
	}

	if len(manifestPlatform.OSFeatures) == 0 && !forceCheck {
		return nil
	}
