...
func ConvertManifest(ctx context.Context, store content.Store, desc imagespec.Descriptor) (imagespec.Descriptor, error) {
	...
	if err != nil {
		return imagespec.Descriptor{}, fmt.Errorf("failed to read index data: %w", err)
	}

	var manifest imagespec.Manifest
	if err := json.Unmarshal(mb, &manifest); err != nil {
		return imagespec.Descriptor{}, fmt.Errorf("failed to unmarshal data into manifest: %w", err)
	}
	...
	if err != nil {
		return imagespec.Descriptor{}, fmt.Errorf("failed to marshal manifest: %w", err)
	}
	...
	for i, c := range append([]imagespec.Descriptor{manifest.Config}, manifest.Layers...) {
	...
	if err := content.WriteBlob(ctx, store, ref, bytes.NewReader(data), desc, content.WithLabels(labels)); err != nil {
		return imagespec.Descriptor{}, fmt.Errorf("failed to update content: %w", err)
	}
	return desc, nil
}
