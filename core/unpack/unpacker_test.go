...
func generateRandomDiffIDs(num int) []digest.Digest {
	diffIDs := make([]digest.Digest, num)
	for i := 0; i < num; i++ {
		diffIDs[i] = digest.FromString(fmt.Sprintf("sha256:%d", i))
	}
	return diffIDs
}
...
func BenchmarkUnpackWithChainID(b *testing.B) {
	...
	for _, sz := range numLayers {
		b.Run(fmt.Sprintf("num of layers: %d", sz), func(b *testing.B) {
			diffIDs := generateRandomDiffIDs(sz)
			for i := 0; i < b.N; i++ {
