...
type failingSnapshotter struct {
	...
	canceledFailKeys map[string]bool // keys that should fail with context.Canceled
}

func (f *failingSnapshotter) Remove(ctx context.Context, key string) error {
	if f.canceledFailKeys != nil && f.canceledFailKeys[key] {
...
// cancelingFailSnapshotter wraps a snapshotter to fail on specific key and cancel context
type cancelingFailSnapshotter struct {
	snapshots.Snapshotter
	cancelFunc func()
	failKey    string
}

func (c *cancelingFailSnapshotter) Remove(ctx context.Context, key string) error {
	if key == c.failKey {
		// Cancel context before returning the error
		c.cancelFunc()
		return fmt.Errorf("simulated remove failure for %s", key)
	}
	return c.Snapshotter.Remove(ctx, key)
}
...
func TestSnapshotGarbageCollectWithFailures(t *testing.T) {
	...
	t.Run("ContextCanceled", func(t *testing.T) {
		// Test that context cancellation is properly detected and returns early
		ctx, db := testDB(t, withSnapshotter("tmp", func(string) (snapshots.Snapshotter, error) {
			return NewTmpSnapshotter(), nil
		}))
		...
		// Run GC with canceled context
		_, err = ss.garbageCollect(canceledCtx)
		// Should return context.Canceled immediately from ctx.Err() check
		if err != context.Canceled {
			t.Errorf("Expected context.Canceled error, got: %v", err)
		}
	})

	t.Run("ContextCanceledWithPartialFailures", func(t *testing.T) {
		// Test that partial failures are reported even when context is canceled midway
		ctx, db := testDB(t, withSnapshotter("tmp", func(string) (snapshots.Snapshotter, error) {
			return NewTmpSnapshotter(), nil
		}))
		...
		// Create Root1 -> A (will fail) and Root2 -> B (will be skipped due to cancellation)
		...
		// Get the internal snapshotter and make A fail
		ss := db.ss["tmp"]
		var aKey string
		ss.Snapshotter.Walk(ctx, func(ctx context.Context, info snapshots.Info) error {
			parts := strings.Split(info.Name, "/")
		...
		if aKey == "" {
			t.Fatal("Could not find A's backend key")
		}

		// Create a context that will be canceled after first root is processed
		cancelableCtx, cancel := context.WithCancel(ctx)

		// Create a custom snapshotter that fails on A and cancels context after
		cfs := &cancelingFailSnapshotter{
			Snapshotter: ss.Snapshotter,
			cancelFunc:  cancel,
			failKey:     aKey,
		}
		...
		// Run GC
		_, err = ss.garbageCollect(cancelableCtx)
		if err == nil {
			t.Fatal("Expected error due to removal failure")
		}

		// Should contain the failed snapshot info
		// Note: Since the failure happens during the last root processing,
		// the error will be about the failed snapshot, not the context cancellation
		errMsg := err.Error()
		if !strings.Contains(errMsg, "A") {
			t.Errorf("Expected error to contain failed snapshot 'A', got: %v", errMsg)
		}
	})
