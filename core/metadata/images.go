...
func (s *imageStore) Create(ctx context.Context, image images.Image) (images.Image, error) {
	...
	if err := update(ctx, s.db, func(tx *bolt.Tx) error {
		...
		// If CreatedAt is not set (zero value), determine it from context or use current time.
		// If CreatedAt is already set by the caller, use that value.
		if image.CreatedAt.IsZero() {
			...
			// Use the provided CreatedAt timestamp
