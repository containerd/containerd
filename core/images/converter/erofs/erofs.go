...
func WithTarIndex(enable bool) ConvertOpt {
	return func(opts *convertOptions) {
		opts.useTarIndex = enable
	}
}
...
func LayerConvertFunc(opts ...ConvertOpt) converter.ConvertFunc {
	return func(ctx context.Context, cs content.Store, desc ocispec.Descriptor) (*ocispec.Descriptor, error) {
		...
		if convertOpts.useTarIndex {
			if err := erofsutils.GenerateTarIndexAndAppendTar(ctx, sr, blobPath, convertOpts.mkfsExtraOpts); err != nil {
				return nil, fmt.Errorf("failed to generate tar index: %w", err)
			}
			...
			log.G(ctx).Debugf("converted %s to EROFS using full conversion mode", desc.Digest)
		}
