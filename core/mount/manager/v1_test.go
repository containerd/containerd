...
// bucket at all, matching one interrupted before completion. system
// mirrors what v1 actually recorded for the mounts it left for the
// caller to perform: full type, source, target and options, unlike
// active above.
...
// this schema would have, using v1's own key names (v1Key*, see
// v1.go) rather than any of buckets.go's "v2" ones. It exists only to
// build fixtures for tests in this file: production code never writes
// this bucket.
...
// when nothing asks for it to be removed, the same as a v2 activation
// (see TestReconcileRemovesUnrealizedActivation): reconciliation
// checks every activation in both schemas, not only ones a caller has
...
func TestV1Reconciled(t *testing.T) {
	...
	// Not marked live on the handler and never explicitly removed:
	// matches wreckage left by an unclean shutdown, discovered only
	// because reconciliation looks.
...
// TestV1ReconciledIncomplete verifies that a v1 activation
// interrupted before it ever completed, with no active bucket at
// all, is released by reconciliation, the same as TestV1Reconciled's
// activation with a dead mount: staleCollision already treats this
// case as not live, not vacuously live, since it is v1's own
// completion marker (see v1HasActive), and reconciliation must agree
// with that rather than leave incomplete wreckage in place forever
// just because it happens to have no positions to find dead.
...
// TestV1ReconciledEmptyActiveIsLive is TestV1ReconciledIncomplete's
// counterpart: an active bucket which exists but happens to have no
// positions in it (every mount in the chain left to the caller, none
// managed by v1 itself) is a real state v1 itself always wrote once
// an activation completed, however few positions it managed, and
// must be told apart from one interrupted before ever completing at
// all, which has no active bucket to speak of. v1HasActive is the
// only thing which can tell the two apart: v1Positions returns nil
// for both.
...
// with, in call order. noopHandler cannot stand in for this: its
// Unmount always succeeds and tracks only whether each point is
// currently live, not the order calls arrive in, so it cannot tell a
// correct unmount order from a reversed one.
...
// activation found orphaned on disk, with no database record left at
// all, is unmounted top of the chain first and base last: the same
// order v1Unmount already gets right when reading positions back from
// the database (positions are written and read back in mount order
// there; see v1Positions), which the orphan directory scan must
// independently reconstruct from v1's on-disk numbering instead,
// where the direction is reversed: base = highest n, top = n = 1 (see
// v1OrphanPositions).
//
// 11 positions, not 2 or 3: v1's own zero-padding (formatMP/formatType
// in the schema this replaced) changes width at 10 and 100, and a
// lexical sort of "1" through "11" would put "10" and "11" ahead of
// "2" through "9". Only sorting numerically on n gets every chain
// length right; this pins that down instead of only the case small
// enough for a lexical sort to get right by accident.
func TestV1OrphanDirsUnmountsTopFirst(t *testing.T) {
	...
	require.NoError(t, os.MkdirAll(orphan, 0700))
	// dir "1" is the top of the chain, mounted last; dir "n" is the
	// base, mounted first (see v1OrphanPositions). Building these
	// ascending from 1 therefore lists them top first: exactly the
	// unmount order this test expects back.
...
func TestV1NeverModified(t *testing.T) {
	...
	// Marked live on the handler: reconciliation, exercised by
	// StartCollection/Finish below, must leave a still-mounted v1
	// activation alone, the same as it leaves a live v2 one alone.
	// This is what "untouched" actually tests now that garbage
	// collection looks at every v1 activation, not just ones marked
	// for removal.
	mp := "/legacy/untouched"
	handler.live = map[string]struct{}{mp: {}}
	seedV1(t, mm.db, "test", v1Activation{
