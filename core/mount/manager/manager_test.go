...
// claimed by the caller is left unapplied at the boundary mount
// (mounts[firstSystemMount]) once resolved through the schema v2
// write transaction, not just in planActivation's own unit tests:
// resolvePosition and rewritePosition must see the same claimed
// suffix planActivation computed, sliced to applyCount, rather than
// the whole transform chain.
func TestActivateHonorsClaimedTransformSuffix(t *testing.T) {
	ctx := namespaces.WithNamespace(context.Background(), "test")
	m, _ := mkTestManager(t)

	// No handler claims "overlay", so the whole mount is left for the
	// caller; "mkdir" is claimed too, so firstSystemMount lands on
	// this one mount with only "format" left for the manager to
	// apply.
