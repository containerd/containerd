...
// readID reads the numeric id an activation, in either schema, was
// created with. Both schemas use the same key for it.
...
func v1HasActive(bkt *bolt.Bucket) bool {
	return bkt.Bucket(bucketKeyActive) != nil
}

// v1Positions reads a v1 activation's mount chain, in order. It
// returns nil both when there is no active bucket at all and when
...
func v1Positions(bkt *bolt.Bucket) []v1Position {
	...
	abkt.ForEachBucket(func(k []byte) error {
		cur := abkt.Bucket(k)
		p := v1Position{
			mtype: string(cur.Get(bucketKeyType)),
			point: string(cur.Get(bucketKeyMountPoint)),
		}
		if v := cur.Get(bucketKeyMountedAt); len(v) > 0 {
			var at time.Time
...
// v1ActivationInfo builds ActivationInfo for a v1 activation. v1
// never recorded system mounts separately from active ones, so Active
// is populated from the mount chain alone; an interrupted activation,
// with no active bucket, is reported with an empty Active list rather
// than as an error.
...
func v1Release(tx *bolt.Tx, namespace, name string) (positions []v1Position, mid uint64, found bool, err error) {
	...
	if lid := bkt.Get(bucketKeyLease); len(lid) > 0 {
...
func v1All(tx *bolt.Tx, fn func(namespace, name string)) {
	...
	for nsk, nsv := nsc.First(); nsk != nil; nsk, nsv = nsc.Next() {
		...
		mbkt := v1bkt.Bucket(nsk).Bucket(bucketKeyMounts)
		if mbkt == nil {
			continue
		}
		mc := mbkt.Cursor()
...
func v1ApplyRemoveNamespace(tx *bolt.Tx, ns string, nsbkt *bolt.Bucket, removed map[string]struct{}) (released []v1Released, remainingMids map[uint64]struct{}, err error) {
	...
	for mk, mv := mc.First(); mk != nil; mk, mv = mc.Next() {
		...
		remainingMids[readID(mbkt.Bucket(mk))] = struct{}{}
	}
...
// itself, named "<n>". Entries are sorted so that, read in reverse by
// v1Unmount, the highest numbered position is unmounted first: the
// same order v1 itself always unmounted them in.
func v1OrphanPositions(mm *mountManager, dir string) ([]v1Position, error) {
	...
	for _, e := range entries {
		...
		mtype, err := os.ReadFile(filepath.Join(full, e))
		if err != nil {
			return nil, err
		}
		positions = append(positions, v1Position{
			mtype: string(mtype),
			point: filepath.Join(full, name),
		})
	}
