...
func TestWarningHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name:             "Resolve",
			resolve:          true,
			expectedWarnings: []string{"Manifest access warning"},
		},
		{
			name:             "Fetch manifest",
		...
		},
		{
			...
			expectedWarnings: []string{"Blob access warning", "Blob access warning"},
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			resolver := NewResolver(ResolverOptions{
				WarningHandler: func(warning string) { warnings = append(warnings, warning) },
			})
			...
			for i, w := range warnings {
				if w != tc.expectedWarnings[i] {
					t.Fatalf("Expected warning to contain %q, got: %s", tc.expectedWarnings[i], w)
				}
			}
		})
	}
...
// Fuzz to make sure we don't panic on malformed input
func FuzzParseWarningText(f *testing.F) {
	f.Add(`299 - "message"`)
	f.Add(`299 - "quoted \"text\" inside"`)
	f.Fuzz(func(t *testing.T, data string) {
		parseWarningText(data)
	})
}
