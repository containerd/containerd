...
func validateProxy(p string) (*url.URL, error) {
	...
	if err != nil {
		...
		if !ok {
			return nil, fmt.Errorf("unable to parse configured proxy URL")
		}
		return nil, fmt.Errorf("unable to parse configured proxy URL: %w", urlErr.Unwrap())
	}
	redacted := redactProxyURL(proxyURL)
	if proxyURL.Hostname() == "" {
		return nil, fmt.Errorf("invalid proxy URL %q: must have a non-empty host", redacted)
	}
	// Mirror the proxy schemes supported by net/http.Transport.
	switch proxyURL.Scheme {
	case "http", "https", "socks5", "socks5h":
	default:
		return nil, fmt.Errorf("invalid proxy URL %q: unsupported scheme %q, must be one of http, https, socks5, socks5h", redacted, proxyURL.Scheme)
	}
