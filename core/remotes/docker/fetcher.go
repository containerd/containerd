...
func (r dockerFetcher) Fetch(ctx context.Context, desc ocispec.Descriptor) (io.ReadCloser, error) {
	...
	return newHTTPReadSeeker(desc.Size, func(offset int64) (io.ReadCloser, error) {
		...
		for _, us := range desc.URLs {
			...
			rc, err := r.open(ctx, req, desc.MediaType, offset, true)
			if err != nil {
...
func (r dockerFetcher) createGetReq(ctx context.Context, host RegistryHost, remainingHosts bool, mediatype string, ps ...string) (*request, int64, error) {
	...
	headResp, err := headReq.doWithRetries(ctx, nil, remainingHosts)
	if err != nil {
		return nil, 0, err
	}
...
func (r dockerFetcher) FetchByDigest(ctx context.Context, dgst digest.Digest, opts ...remotes.FetchByDigestOpts) (io.ReadCloser, ocispec.Descriptor, error) {
	...
	seeker, err := newHTTPReadSeeker(sz, func(offset int64) (io.ReadCloser, error) {
		return r.open(ctx, getReq, config.Mediatype, offset, false)
	})
	if err != nil {
		return nil, desc, err
	}
...
func (r dockerFetcher) open(ctx context.Context, req *request, mediatype string, offset int64, remainingHosts bool) (_ io.ReadCloser, retErr error) {
	if mediatype == "" {
	...
	resp, err := req.doWithRetries(ctx, nil, remainingHosts)
	if err != nil {
		return nil, err
	}
	defer func() {
