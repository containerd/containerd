...
func (r *request) retryRequest(ctx context.Context, responses []*http.Response, lastHost bool) (bool, error) {
	...
	case http.StatusTooManyRequests:
		...
		if retryAfterStr := last.Header.Get("Retry-After"); retryAfterStr != "" {
			now := time.Now().UTC()
			if t, ok := parseRetryAfter(now, retryAfterStr); ok && t.After(now.Add(default429MaxRetryAfter)) {
...
func (r *request) delay429(ctx context.Context, responses []*http.Response) error {
	...
	now := time.Now().UTC()
	if t, ok := parseRetryAfter(now, retryAfterStr); ok {
		delay = max(t.Sub(now), 0)
	} else {
...
func parseRetryAfter(now time.Time, header string) (time.Time, bool) {
	header = strings.TrimSpace(header)
	if header == "" {
		return time.Time{}, false
	}
	if seconds, err := strconv.ParseInt(header, 10, 64); err == nil {
		if seconds < 0 || seconds > math.MaxInt64-now.Unix() {
			return time.Time{}, false
		}
		t := time.Unix(now.Unix()+seconds, int64(now.Nanosecond())).UTC()
		if t.Year() > 9999 {
			return time.Time{}, false
		}
		return t, true
