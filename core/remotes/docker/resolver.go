	...
	for _, u := range paths {
		for i, host := range hosts {
			...
			if dgst == "" || size == -1 {
				...
				resp, err := req.doWithRetries(ctx, nil, false)
				if err != nil {
					return "", ocispec.Descriptor{}, err
				}
...
func (r *request) doWithRetries(ctx context.Context, responses []*http.Response, remainingHosts bool) (*http.Response, error) {
	resp, err := r.do(ctx)
	if err != nil {
		return nil, err
	}
	...
	retry, err := r.retryRequest(ctx, responses, remainingHosts)
	if err != nil {
		resp.Body.Close()
		return nil, err
	}
	if retry {
		resp.Body.Close()
		return r.doWithRetries(ctx, responses, remainingHosts)
	}
	return resp, err
}

func (r *request) retryRequest(ctx context.Context, responses []*http.Response, remainingHosts bool) (bool, error) {
	if len(responses) > 5 {
		return false, nil
	}
	...
	case http.StatusServiceUnavailable, http.StatusGatewayTimeout, http.StatusInternalServerError:
		// Do not retry if there are other servers to try or the same error was seen in the last request
		if !remainingHosts && (len(responses) <= 1 || responses[len(responses)-2].StatusCode != last.StatusCode) {
			return true, nil
		}
	}
