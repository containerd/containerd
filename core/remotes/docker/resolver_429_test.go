...
func TestParseRetryAfter(t *testing.T) {
	now := time.Date(2026, time.September, 18, 20, 0, 0, 0, time.UTC)
	tests := []struct {
		name     string
		header   string
		wantOk   bool
		...
		{
			name:     "delta seconds exceeding time.Duration max (~292.5 years)",
			header:   "9223372037",
			wantOk:   true,
			wantTime: time.Unix(now.Unix()+9223372037, int64(now.Nanosecond())).UTC(),
		},
		{
		...
		},
		{
			...
			wantOk:   true,
			wantTime: time.Date(2015, time.October, 21, 7, 28, 0, 0, time.UTC),
		},
		{
			name:     "HTTP date in the future",
			header:   now.Add(60 * time.Second).UTC().Format(http.TimeFormat),
			wantOk:   true,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			retryAt, ok := parseRetryAfter(now, tt.header)
			assert.Equal(t, tt.wantOk, ok)
			if !tt.wantOk {
				assert.True(t, retryAt.IsZero())
				return
			}
			assert.Equal(t, tt.wantTime, retryAt)
		})
	}
}
...
func TestRetry429WithRetryAfterHeader(t *testing.T) {
	...
	t.Run("retries exhausted", func(t *testing.T) {
		...
		assert.WithinDuration(t, time.Now().Add(30*time.Second), *statusErr.RetryAfter, 5*time.Second)

		errStr := err.Error()
		assert.Contains(t, errStr, "(Retry-After: "+statusErr.RetryAfter.Format(time.RFC3339)+")")
...
func TestRetry429ExceedsMaxRetryAfterFailsImmediately(t *testing.T) {
	...
	assert.Equal(t, http.StatusTooManyRequests, statusErr.StatusCode)
	require.NotNil(t, statusErr.RetryAfter)
	assert.WithinDuration(t, time.Now().Add(61*time.Second), *statusErr.RetryAfter, 5*time.Second)
	assert.Contains(t, err.Error(), "(Retry-After: "+statusErr.RetryAfter.Format(time.RFC3339)+")")
